...
function addNewProfileToDatabase(callback, newProfileData) {
    ...
    db.serialize(function () {
        ...
        db.each("SELECT personid FROM Persons WHERE username=?", [newProfileData.username], function (err, row) {
            console.log(row);
            if (err) {
                return callback(err);
            }
            if (row != undefined) {
                console.log("new username is in database");
        ...
        if (newUsernameValid) {
            db.each("SELECT personid FROM Persons WHERE username=?", [oldUserName], function (err, row) {
                console.log(row);
            });
            db.run(`DELETE FROM Persons WHERE username=?`, [newProfileData.oldUserName], function (err) {
                if (err) {
                    return console.error(err.message);
                }
            });
            // insert one row into the langs table
            db.run(`INSERT INTO Persons(personid, firstname, surname, username, password, emailaddress) VALUES(?, ?, ?, ?, ?)`, [personid, newProfileData.firstname, newProfileData.surname, newProfileData.username, newProfileData.password, newProfileData.emailaddress], function (err) {
                if (err) {
